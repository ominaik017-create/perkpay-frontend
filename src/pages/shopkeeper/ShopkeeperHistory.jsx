...
export default function ShopkeeperHistory() {
  ...
  async function markReceived(t) {
    if (!confirm(`Mark ₹${t.amount} from ${t.users?.name || 'this customer'} as received? Only do this if the payment actually reached your account.`)) return;
    try {
      await api.shopkeeperConfirm(t.order_id);
      load();
    } catch (err) {
      alert(err.message);
    }
      ...
      {txns.length === 0 ? (
        ...
        <div key={t.id} className="card" style={{ padding: 14, marginBottom: 8 }}>
          ...
          {t.status === 'pending' && (
            ...
            >
              Mark as received (payment stuck?)
            </button>
          )}
        </div>
      ))}
