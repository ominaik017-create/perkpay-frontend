    ...
    <div className="page-container">
      <div className="scroll-area">
        ...
        <div style={balanceCard}>
          ...
          <h1 style={{ fontSize: 34, marginTop: 4 }}>{formatPoints(user?.points_balance)}</h1>
        ...
        {shopPoints.length === 0 ? (
          ...
          <div key={sp.shop_id} className="card" style={{ padding: 14, marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <p style={{ fontWeight: 700, color: 'var(--brand)' }}>{formatPoints(sp.balance)} pts</p>
          </div>
        ))}
        ...
        {txns.length === 0 ? (
          ...
          <div key={t.id} className="card" style={{ padding: 14, marginBottom: 8, display: 'flex', justifyContent: 'space-between' }}>
            <div>
              ...
              {t.earned_points > 0 && (
                <p style={{ fontSize: 12, color: 'var(--success)', marginTop: 2, fontWeight: 600 }}>+{formatPoints(t.earned_points)} pts earned</p>
              )}
            </div>
